...
const AllCategories = () => {
  ...
  const handleDeleteItem = async () => {
    ...
    return categoriesService.deleteCategory(selectedItem.unique_id, { module_unique_id: moduleId, sub_module_unique_id: subModuleId });
  };
  ...
  const handleApproveItem = async () => {
    ...
    return categoriesService.approveCategory({ unique_id: selectedItem.unique_id }, { module_unique_id: moduleId, sub_module_unique_id: subModuleId });
  };
      ...
      <div className="xui-py-1-half">
        <div className="xui-d-flex xui-flex-ai-center xui-flex-jc-space-between xui-mb-1-half">
          ...
          <div className="xui-d-flex xui-flex-ai-center xui-grid-gap-half">
            ...
            {canAdd && (
              <button onClick={() => router.push('/dashboard/candidate-portal/categories/add')} className="xui-btn xui-font-sz-80 xui-bdr-rad-half xui-font-w-500 xui-d-flex xui-flex-ai-center xui-grid-gap-half" style={{ backgroundColor: 'var(--primary-600)', color: 'var(--secondary-700)' }}>
                        ...
                        {item.Candidate ? (
                          <div className="xui-d-flex xui-flex-ai-center xui-grid-gap-half">
                            {item.Candidate.image ? (
                              <img src={item.Candidate.image} alt={item.Candidate.name} style={{ width: 28, height: 28, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                            ) : (
                              <div style={{ width: 28, height: 28, borderRadius: '50%', backgroundColor: 'var(--neutral-200)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 600, color: 'var(--neutral-600)', flexShrink: 0 }}>
                                {item.Candidate.name.charAt(0)}
                              </div>
                            )}
                            <div>
                              <p className="xui-font-sz-85 xui-font-w-500">{item.Candidate.name}</p>
                              {item.Candidate.state && <p className="xui-font-sz-75 xui-opacity-5">{item.Candidate.state}</p>}
                            </div>
                          </div>
                        ...
                        <td>
                          <div className="xui-tooltip" xui-set="left">
                            ...
                            <div className="xui-tooltip-content xui-flex-ai-center xui-grid-gap-half" style={{ display: 'flex', maxWidth: '500px' }}>
                              ...
                              {canEdit && item.approved_by === null && (
                                <button onClick={() => router.push(`/dashboard/candidate-portal/categories/edit/${item.unique_id}`)} className="xui-btn xui-btn-small xui-d-flex xui-flex-ai-center xui-grid-gap-half xui-cursor-pointer xui-font-sz-80" style={{ backgroundColor: 'var(--info-light)', border: 'none', color: 'var(--info)' }}><Edit size={16} /> Edit</button>
                              )}
                              {canDelete && (
