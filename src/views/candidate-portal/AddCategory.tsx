...
interface FormData { name: string; candidate_unique_id: string; }
...
const AddCategory = () => {
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<FormData>({ defaultValues: { name: '', candidate_unique_id: '' } });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setLoading(true);
    try {
      const response = await categoriesService.addCategory({ name: data.name, candidate_unique_id: data.candidate_unique_id }, { module_unique_id: accessIds.module_unique_id, sub_module_unique_id: accessIds.sub_module_unique_id });
      if (response.success) {
