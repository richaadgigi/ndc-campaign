...
const EditCategory = () => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setTimeout(() => router.push('/dashboard/candidate-portal/categories'), 1500);
  ...
  const handleApproveConfirm = async (): Promise<{ success: boolean; message: string }> => {
    ...
    const response = await categoriesService.approveCategory({ unique_id: id }, { module_unique_id: moduleId, sub_module_unique_id: subModuleId });
      ...
      <div className="xui-py-1">
        ...
        <div className="xui-d-grid xui-grid-col-1 xui-md-grid-col-2 xui-grid-gap-2">
          ...
          <div className="xui-bg-white xui-bdr-rad-half xui-p-1" style={{ border: '1px solid var(--neutral-200)', alignSelf: 'start' }}>
            ...
            {item.Candidate && (
              <div className="xui-d-flex xui-flex-ai-center xui-flex-jc-space-between xui-py-half" style={{ borderBottom: '1px solid var(--neutral-100)' }}>
                <span className="xui-font-sz-80 xui-opacity-5">Candidate</span>
                <div className="xui-d-flex xui-flex-ai-center xui-grid-gap-half">
                  {(item.Candidate as any).image ? (
                    <img src={(item.Candidate as any).image} alt={item.Candidate.name} style={{ width: 22, height: 22, borderRadius: '50%', objectFit: 'cover' }} />
                  ) : (
                    <div style={{ width: 22, height: 22, borderRadius: '50%', backgroundColor: 'var(--neutral-200)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: 600 }}>
                      {item.Candidate.name.charAt(0)}
                    </div>
                  )}
                  <span className="xui-font-sz-80 xui-font-w-500">{item.Candidate.name}</span>
                </div>
              </div>
            )}
            {item.Creator && (
