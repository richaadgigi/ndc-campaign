...
interface FormData { question: string; answer: string; candidate_unique_id: string; }
...
const AddFaq = () => {
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<FormData>({ defaultValues: { question: '', answer: '', candidate_unique_id: '' } });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setLoading(true);
    try {
      const response = await faqsService.add({ question: data.question, answer: data.answer, candidate_unique_id: data.candidate_unique_id }, { module_unique_id: accessIds.module_unique_id, sub_module_unique_id: accessIds.sub_module_unique_id });
      if (response.success) {
        setSuccessMessage('FAQ added successfully'); showAlert('success-alert');
        setTimeout(() => router.push('/dashboard/candidate-portal/faqs'), 1500);
