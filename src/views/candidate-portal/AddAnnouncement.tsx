...
interface FormData { candidate_unique_id: string; title: string; start_date: string; end_date: string; }
...
const AddAnnouncement = () => {
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<FormData>({ defaultValues: { candidate_unique_id: '', title: '', start_date: '', end_date: '' } });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setTimeout(() => router.push('/dashboard/candidate-portal/announcements'), 1500);
  ...
  return (
    <div>
      <Navbar title="Add Announcement" subtitle="Create a new candidate announcement" />
