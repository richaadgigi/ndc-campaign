...
const AllAnnouncements = () => {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      const response = await announcementsService.getAll({ page: currentPage, size: pageSize, module_unique_id: moduleId, sub_module_unique_id: subModuleId });
  ...
  const searchItems = useCallback(async (query: string) => {
    ...
    setLoading(true); setFetchError('');
    try {
      const response = await announcementsService.search({ search: query, page: currentPage, size: pageSize, module_unique_id: moduleId, sub_module_unique_id: subModuleId });
      handleResponse(response);
    } catch (err: any) { setFetchError(extractErrorMessage(err, 'Failed to search announcements')); } finally { setLoading(false); }
  ...
  const filterItems = useCallback(async (range: { start_date: string; end_date: string }) => {
    ...
    setLoading(true); setFetchError('');
    try {
      const response = await announcementsService.filter({ start_date: range.start_date, end_date: range.end_date, page: currentPage, size: pageSize, module_unique_id: moduleId, sub_module_unique_id: subModuleId });
      handleResponse(response);
    } catch (err: any) { setFetchError(extractErrorMessage(err, 'Failed to filter announcements')); } finally { setLoading(false); }
  }, [moduleId, subModuleId, currentPage, pageSize]);

  const handleApproveItem = async () => {
    if (!moduleId || !subModuleId || !selectedItem) return { success: false, message: 'Unable to approve announcement' };
    return announcementsService.approve({ unique_id: selectedItem.unique_id }, { module_unique_id: moduleId, sub_module_unique_id: subModuleId });
  };

  const handleDeleteItem = async () => {
    if (!moduleId || !subModuleId || !selectedItem) return { success: false, message: 'Unable to delete announcement' };
    return announcementsService.remove(selectedItem.unique_id, { module_unique_id: moduleId, sub_module_unique_id: subModuleId });
  };
  ...
  return (
    <div>
      <Navbar title="Announcements" subtitle="Manage candidate announcements" />
      <div className="xui-py-1-half">
        <div className="xui-d-flex xui-flex-ai-center xui-flex-jc-space-between xui-mb-1-half">
          ...
          <div className="xui-d-flex xui-flex-ai-center xui-grid-gap-half">
            ...
            {canAdd && (
              <button onClick={() => router.push('/dashboard/candidate-portal/announcements/add')} className="xui-btn xui-font-sz-80 xui-bdr-rad-half xui-font-w-500 xui-d-flex xui-flex-ai-center xui-grid-gap-half" style={{ backgroundColor: 'var(--primary-600)', color: 'var(--secondary-700)' }}>
                <span className="icon-container"><Add size={16} /></span> Add Announcement
              </button>
            )}
          </div>
        </div>
        ...
        <div className="xui-bg-white xui-bdr-rad-half xui-overflow-hidden" style={{ border: '1px solid var(--neutral-200)' }}>
          <div className="xui-table-responsive">
            {loading ? (
              ...
              <EmptyState title="No announcements found" message={searchQuery ? "No announcements match your search query." : hasActiveFilters ? "No announcements match your filter criteria." : "There are no announcements to display."} />
            ) : (
                        ...
                        <td>
                          <div className="xui-tooltip" xui-set="left">
                            ...
                            <div className="xui-tooltip-content xui-flex-ai-center xui-grid-gap-half" style={{ display: 'flex', maxWidth: '500px' }}>
                              ...
                              {canEdit && item.approved_by === null && (
                                <button onClick={() => router.push(`/dashboard/candidate-portal/announcements/edit/${item.unique_id}`)} className="xui-btn xui-btn-small xui-d-flex xui-flex-ai-center xui-grid-gap-half xui-cursor-pointer xui-font-sz-80" style={{ backgroundColor: 'var(--info-light)', border: 'none', color: 'var(--info)' }}><Edit size={16} /> Edit</button>
                              )}
                              {canDelete && (
