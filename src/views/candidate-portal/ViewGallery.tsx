...
const ViewGallery = () => {
  ...
  const fetchItem = useCallback(async () => {
    ...
    try {
      const response = await galleryService.get(id, { module_unique_id: moduleId, sub_module_unique_id: subModuleId });
  ...
  const handleSaveImage = async () => {
    ...
    setSaving(true);
    try {
      const response = await galleryService.editImage({ unique_id: item.unique_id, image: editImage, image_public_id: editImagePublicId }, { module_unique_id: moduleId, sub_module_unique_id: subModuleId });
  ...
  const handleDeleteItem = async () => {
    ...
    return galleryService.delete(item.unique_id, { module_unique_id: moduleId, sub_module_unique_id: subModuleId });
  };
  ...
  return (
    <div>
      ...
      <ConfirmModal id="delete-modal" title="Delete Image" message="Are you sure you want to delete this gallery image?" itemName={item?.title || 'this image'} confirmText="Delete" confirmingText="Deleting..." confirmButtonStyle="danger" onConfirm={handleDeleteItem} onSuccess={() => router.push('/dashboard/candidate-portal/gallery')} setError={setActionError} setSuccessMessage={setSuccessMessage} showAlert={showAlert} />
    </div>
  );
};
