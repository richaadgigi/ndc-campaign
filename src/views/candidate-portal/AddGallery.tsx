...
interface FormData { title: string; tags: string[]; candidate_unique_id: string; }
...
const AddGallery = () => {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<FormData>({ defaultValues: { title: '', tags: [], candidate_unique_id: '' } });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setTimeout(() => router.push('/dashboard/candidate-portal/gallery'), 1500);
