...
export interface CampaignStats {
  ...
  total_candidates_via_positions: { Position: { name: string; stripped: string }; total_count: number }[];
  total_members_via_member_role: { MemberRole: { name: string; stripped: string } | null; total_count: number }[];
  total_member_role_acls_via_member_role: { MemberRole: { name: string } | null; total_count: number }[];
  total_member_role_acls_via_module: { Module: { name: string } | null; total_count: number }[];
  total_member_role_acls_via_sub_module: { SubModule: { name: string } | null; total_count: number }[];
}
...
export interface CandidatePortalStatsResponse {
  success: boolean;
  message: string;
  data: CandidatePortalStats | null;
}
...
export const analyticsService = {
  ...
  getCampaignStats: async (params: ModuleParams): Promise<CampaignStatsResponse> => {
    const query = buildQueryParams(params);
    const response = await api.get(`/user/campaign/stats?${query}`);
    return response.data;
  },
